...
def receive(sources, timeout=-1):
    ...
    When invoked by the same entity E (where E can be an actor, task or driver),
    for each source S in sources, this function returns all signals generated by
    S since the last receive() was invoked by E on S. If this is
    ...
    on same source S will get independent copies of the signals generated by S.

    Args:
        sources: List of sources from which caller waits for signals.
            A source is either an object ID returned by a task (in this case the
            object ID is used to IDentify that task), or an actor handle.
        ...
        timeout: Maximum time (in seconds) this function waits to get a signal
            from a source in sources. If -1, the timeout is "infinity".

    Returns:
        A list of pairs (S, sig), where S is a source in the sources argument,
        and sig is a signal generated by S since the last time receive() was
        called on S. Thus, for each S in sources, the return list can contain
        zero or multiple entries.
    """

    # If -1, initialize the timeout to a huge value (i.e., over 30,000 years
