...
def receive(sources, timeout=10**12):
    ...
    When invoked by the same task or actor, for each source S, this function
    returns all signals associated to S since the last receive() or forget()
    were invoked on S. If this is the first call on S, this function returns
    all past signals generated by S so far. Note that two different actors or
    tasks that call receive() on same source S will get same signals.
