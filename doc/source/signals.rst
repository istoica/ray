...
be received by other tasks and actors. In addition, task failures or actor
...
it returns.	object ID return values.
...
.. code-block:: python
  ...
  # The signal is a tuple where the first element is the object ID
  # returned by the task that we passed in the signal.receive(),
